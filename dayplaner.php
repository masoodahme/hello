       ...
       <div class="col-sm-2">
        ...
        <div class="col-sm-2">
            <span class="btn btn-primary"><?php 
                ...
                $date=date('l') ;
                echo mysqli_num_rows(mysqli_query($con,"select * from addtask where `done`='1'")); ?> Done</span>
        </div>
        <div class="col-sm-2">
           <span class="btn btn-primary"><?php
                date_default_timezone_set('Asia/Kolkata');
                $date=date('l') ;
                echo mysqli_num_rows(mysqli_query($con,"select * from addtask where `done`='0' and day='$date'")); ?> Remaning</span>
        </div>                 
        <div class="col-sm-2">
            <button type="button" class="btn btn-primary" data-toggle="collapse" name="task" class="btn btn-primary" data-target="#pendos"><?php
                date_default_timezone_set('Asia/Kolkata');
                $date=date('l') ;
                echo mysqli_num_rows(mysqli_query($con,"select * from addtask where `done`='0'")); ?>  Pending</button>
        </div>
    </div>
    <br>
    <br>
      ...
      <table class="table ">
       ...
       <tr>
        <?php
           date_default_timezone_set('Asia/Kolkata');
           $month=date('l');
           $query="SELECT `name`, `date`, `month`, `day` FROM `addtask` WHERE `day`='$month'";
      ...
      <table class="table ">
       ...
       <tr>
        <?php
           date_default_timezone_set('Asia/Kolkata');
           $day=date('l');
           $query="SELECT `name`, `date`, `day` FROM `addtask` WHERE  `done`='1'";
    $result=mysqli_query($con,$query);
        //echo "could not insert".mysqli_error($con).mysqli_errno($con);
  //echo $query; 
     if(mysqli_num_rows($result)>=1)
      ...
      <table class="table ">
       ...
       <tr>
        <?php
           date_default_timezone_set('Asia/Kolkata');
           $day=date('l');
           $query="SELECT `name`, `date`, `day` FROM `addtask` WHERE  `done`='0'";
    $result=mysqli_query($con,$query);
        //echo "could not insert".mysqli_error($con).mysqli_errno($con);
  //echo $query; 
     if(mysqli_num_rows($result)>=1)
